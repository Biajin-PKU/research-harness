...
def test_paper_link_to_new_topic(runner):
    ...
    runner.invoke(main, ["paper", "ingest", "--title", "P1", "--arxiv-id", "2401.60001", "--topic", "t1"])
    result = runner.invoke(main, ["--json", "paper", "link", "1", "--topic", "t2", "--relevance", "high"])
...
def test_paper_link_updates_relevance(runner):
    runner.invoke(main, ["topic", "init", "t1"])
    runner.invoke(main, ["paper", "ingest", "--title", "P1", "--arxiv-id", "2401.60002", "--topic", "t1", "--relevance", "low"])
    result = runner.invoke(main, ["--json", "paper", "link", "1", "--topic", "t1", "--relevance", "high"])
...
def test_paper_link_nonexistent_topic(runner):
    runner.invoke(main, ["topic", "init", "t1"])
    runner.invoke(main, ["paper", "ingest", "--title", "P1", "--arxiv-id", "2401.60003", "--topic", "t1"])

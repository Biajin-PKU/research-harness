...
def _setup_topic_with_papers(runner: CliRunner, topic: str = "llm-safety") -> dict:
    ...
    runner.invoke(main, [
        "paper", "ingest",
        "--title", "Needs Annotation Paper",
        "--arxiv-id", "2401.00002",
        "--pdf-path", "/tmp/needs_annotate.pdf",
        "--topic", topic,
    ])
...
def test_task_generate_creates_tasks_for_actionable_papers(runner):
    ...
    result = runner.invoke(main, ["--json", "task", "generate", "--topic", "llm-safety"])
    assert result.exit_code == 0, result.output
    data = json.loads(result.output)
    assert data["created_count"] >= 2  # at least annotate + attach_pdf
    assert data["skipped_count"] >= 0
    # Verify tasks exist
    tasks_result = runner.invoke(main, ["--json", "task", "list", "--topic", "llm-safety"])
...
def test_task_generate_skips_duplicates(runner):
    ...
    _setup_topic_with_papers(runner)
    result1 = runner.invoke(main, ["--json", "task", "generate", "--topic", "llm-safety"])
    assert result1.exit_code == 0
    data1 = json.loads(result1.output)
    created_first = data1["created_count"]

    result2 = runner.invoke(main, ["--json", "task", "generate", "--topic", "llm-safety"])
...
def test_task_generate_respects_dry_run(runner):
    ...
    _setup_topic_with_papers(runner)
    result = runner.invoke(main, ["--json", "task", "generate", "--topic", "llm-safety", "--dry-run"])

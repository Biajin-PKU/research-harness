...
class TestTitleNormalization:
    ...
    def test_remove_punctuation(self):
        assert (
            _normalize_title("A: Survey of Methods (2024)")
            == "a survey of methods 2024"
        )
...
class TestDuplicateDetection:
    def test_finds_similar_title(self, db, conn):
        ...
        _insert_paper(
            conn, 2, "Deep Reinforcement Learning for Bidding Optimization in Ads"
        )
        conn.commit()

        candidates = _find_duplicate_candidates(
            conn,
            "Deep Reinforcement Learning for Bidding Optimization in Ads",
            exclude_id=2,
        )
    ...
    def test_topic_scoped(self, db, conn):
        ...
        conn.execute(
            "INSERT INTO paper_topics (paper_id, topic_id, relevance) VALUES (2, 2, 'high')"
        )
        conn.commit()

        # Scoped to topic 1 — should only find paper 1
        candidates = _find_duplicate_candidates(
            conn, "Budget Allocation Methods Extended", topic_id=1
        )
...
class TestSurveyDetection:
    def test_detects_survey_title(self, db, conn):
        ...
        result = _detect_survey_paper(
            conn, 1, "A Survey of Auto-Bidding Methods", topic_id=1
        )
    ...
    def test_no_flag_for_normal_paper(self, db, conn):
        ...
        result = _detect_survey_paper(
            conn, 1, "Budget Pacing for Online Ads", topic_id=1
        )
    ...
    def test_meta_analysis(self, db, conn):
        ...
        assert (
            _detect_survey_paper(
                conn, 1, "A Meta-Analysis of Ad Auction Results", topic_id=1
            )
            is True
        )
...
class TestPaperMoveCLI:
    def test_move_paper(self, runner, tmp_path, monkeypatch):
        ...
        result = runner.invoke(
            main, ["paper", "move", "1", "--from-topic", "src", "--to-topic", "dst"]
        )
        ...
        conn = db.connect()
        in_dst = conn.execute(
            "SELECT * FROM paper_topics WHERE paper_id = 1 AND topic_id = 2"
        ).fetchone()
        in_src = conn.execute(
            "SELECT * FROM paper_topics WHERE paper_id = 1 AND topic_id = 1"
        ).fetchone()
        conn.close()
...
class TestPaperBulkUpdateCLI:
    def test_bulk_update_relevance(self, runner, tmp_path, monkeypatch):
        ...
        conn.execute(
            "INSERT INTO paper_topics (paper_id, topic_id, relevance) VALUES (1, 1, 'medium'), (2, 1, 'medium')"
        )
        conn.commit()
        conn.close()
        ...
        conn = db.connect()
        rows = conn.execute(
            "SELECT relevance FROM paper_topics WHERE topic_id = 1"
        ).fetchall()
        conn.close()
...
class TestTopicStatsCLI:
    def test_stats_output(self, runner, tmp_path, monkeypatch):
        ...
        conn.execute(
            "INSERT INTO paper_topics (paper_id, topic_id, relevance) VALUES (1, 1, 'high'), (2, 1, 'medium')"
        )
        conn.commit()
        conn.close()

...
from research_harness.pdf_download import PaperDownloadCandidate, build_candidate_urls, preferred_filename
...
def test_arxiv_abs_url_expands_to_pdf() -> None:
    candidate = PaperDownloadCandidate(paper_id=1, title="Test", url="https://arxiv.org/abs/2302.01523")
    urls = build_candidate_urls(candidate)
    assert "https://arxiv.org/pdf/2302.01523.pdf" in urls
...
def test_doi_always_generates_doi_org_redirect() -> None:
    candidate = PaperDownloadCandidate(paper_id=24, title="Any Paper", doi="10.9999/test")
    urls = build_candidate_urls(candidate)
...
def test_scihub_after_oa_before_publisher(monkeypatch) -> None:
    ...
    monkeypatch.setattr("research_harness.pdf_download._unpaywall_oa_url", lambda doi: None)
    ...
    assert scihub_idx < acm_idx, f"Sci-Hub ({scihub_idx}) should be before ACM ({acm_idx})"
    assert scihub_idx < doi_idx, f"Sci-Hub ({scihub_idx}) should be before doi.org ({doi_idx})"
...
def test_extract_scihub_pdf_url_meta_citation() -> None:
    ...
    from research_harness.acquisition.downloader import _extract_scihub_pdf_url
    html = b'<meta name="citation_pdf_url" content="/storage/2024/7562/abc/shen2019.pdf">'
    result = _extract_scihub_pdf_url(html)

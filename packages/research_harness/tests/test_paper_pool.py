...
def test_ingest_is_idempotent_and_updates_topic_link(conn):
    ...
    first_id = pool.ingest(
        Paper(title="Attention Is All You Need", arxiv_id="1706.03762", authors=["A"], year=2017, venue="NeurIPS"),
        topic_id=topic_id,
        relevance="high",
    )
    second_id = pool.ingest(
        Paper(title="Attention Is All You Need", arxiv_id="1706.03762", authors=["A", "B"], year=2017, venue="NeurIPS"),
        topic_id=topic_id,
        relevance="medium",
    )
...
def test_annotations_upsert(conn):
    ...
    pool = PaperPool(conn)
    pool.upsert_annotation(PaperAnnotation(paper_id=paper_id, section="summary", content="a", source="test"))
    pool.upsert_annotation(PaperAnnotation(paper_id=paper_id, section="summary", content="b", source="test2"))
...
def test_topic_notes_upsert_and_filter(conn):
    ...
    note_id = pool.upsert_topic_note(
        TopicPaperNote(paper_id=paper_id, topic_id=topic_id, note_type='relevance', content='first', source='codex')
    )
    pool.upsert_topic_note(
        TopicPaperNote(paper_id=paper_id, topic_id=topic_id, note_type='relevance', content='updated', source='claude')
    )
...
def test_topic_note_requires_existing_topic_link(conn):
    ...
    try:
        pool.upsert_topic_note(
            TopicPaperNote(paper_id=paper_id, topic_id=topic_id, note_type='relevance', content='x', source='codex')
        )

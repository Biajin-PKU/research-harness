...
class TestRebuttalFormat:
    def test_formats_rebuttal(self, db, conn):
        ...
        conn.execute("INSERT INTO projects (id, topic_id, name) VALUES (1, 1, 'test-project')")
        conn.execute(
        ...
        with patch("research_harness.execution.llm_primitives._get_client") as mock_client, \
             patch("research_harness.execution.llm_primitives._client_chat", return_value=mock_rebuttal):
    ...
    def test_no_issues(self, db, conn):
        ...
        conn.execute("INSERT INTO projects (id, topic_id, name) VALUES (1, 1, 'empty-project')")
        conn.commit()
...
class TestTopicExport:
    def test_generates_markdown_report(self, db, conn):
        conn.execute("INSERT INTO topics (id, name, description) VALUES (1, 'auto-bidding', 'Research on auto bidding')")
        _insert_paper(conn, 1, "DQN Bidding", year=2023, venue="NeurIPS", citation_count=50)
        _insert_paper(conn, 2, "PPO Bidding", year=2024, venue="ICML", citation_count=20)
    ...
    def test_with_taxonomy_and_contradictions(self, db, conn):
        ...
        conn.execute("INSERT INTO paper_topics (paper_id, topic_id, relevance) VALUES (1, 1, 'high')")
        conn.execute("INSERT INTO taxonomy_nodes (id, topic_id, name, description) VALUES (1, 1, 'DQN', 'Deep Q-Network')")
        conn.execute(
...
class TestVisualizeTopic:
    def test_taxonomy_tree(self, db, conn):
        conn.execute("INSERT INTO topics (id, name) VALUES (1, 'test')")
        conn.execute("INSERT INTO taxonomy_nodes (id, topic_id, name) VALUES (1, 1, 'RL')")
        conn.execute("INSERT INTO taxonomy_nodes (id, topic_id, name, parent_id) VALUES (2, 1, 'DQN', 1)")
        conn.execute("INSERT INTO taxonomy_nodes (id, topic_id, name, parent_id) VALUES (3, 1, 'PPO', 1)")
        conn.commit()

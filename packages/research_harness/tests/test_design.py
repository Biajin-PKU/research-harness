...
class TestDesignBriefExpand:
    def test_basic_expansion(self, db, conn):
        ...
        mock_response = json.dumps({
            ...
            "method_slots": [
                {"name": "policy_backbone", "role": "Generates bidding actions", "candidates": ["PPO", "SAC"], "status": "open"},
                {"name": "budget_allocator", "role": "Distributes budget across campaigns", "candidates": ["LP", "DP"], "status": "open"},
                {"name": "state_encoder", "role": "Encodes market state", "candidates": ["Transformer", "GRU"], "status": "open"},
            ],
        ...
        with patch("research_harness.execution.llm_primitives._get_client") as mock_client, \
            ...
            result = design_brief_expand(db=db, topic_id=1, direction="Scalable RL for budget allocation")
    ...
    def test_with_constraints(self, db, conn):
        ...
        mock_response = json.dumps({
            ...
            "method_slots": [{"name": "encoder", "role": "encode", "candidates": ["X"], "status": "open"}],
            "blocking_questions": [],
        })
    ...
    def test_malformed_response_graceful(self, db, conn):
        ...
        mock_response = json.dumps({
            ...
            "method_slots": ["not a dict", 42, {"name": "valid", "role": "r", "candidates": [], "status": "open"}],
        })
...
class TestDesignGapProbe:
    def test_basic_probe(self, db, conn):
        ...
        brief = {
            ...
            "method_slots": [
                {"name": "encoder", "role": "encode state", "candidates": ["Transformer", "GRU"], "status": "open"},
                {"name": "decoder", "role": "decode action", "candidates": [], "status": "blocked"},
            ],
        }

        mock_response = json.dumps({
            "knowledge_gaps": [
                {"slot": "decoder", "gap_type": "technique_unknown", "severity": "critical",
                 "search_query": "action decoder RL bidding", "candidate_paper_ids": [1]},
            ],
    ...
    def test_with_method_inventory_override(self, db, conn):
        ...
        mock_response = json.dumps({
            "knowledge_gaps": [
                {"slot": "planner", "gap_type": "performance_unclear", "severity": "moderate",
                 "search_query": "planning under uncertainty", "candidate_paper_ids": []},
            ],
...
class TestAlgorithmCandidateGenerate:
    def test_generates_candidates(self, db, conn):
            ...
            "candidates": [
                {
                    ...
                    "components": [
                        {"name": "state_encoder", "role": "encode market state",
                         "provenance_tag": "borrowed", "source_paper_id": 1, "details": "From paper 1"},
                        {"name": "action_decoder", "role": "decode bidding action",
                         "provenance_tag": "novel", "source_paper_id": None, "details": "New design"},
                    ],
                ...
                },
                {
                    ...
                    "components": [
                        {"name": "planner", "role": "plan budget allocation",
                         "provenance_tag": "modified", "source_paper_id": 2, "details": "Modified from paper 2"},
                    ],
        ...
        with patch("research_harness.execution.llm_primitives._get_client") as mock_client, \
             patch("research_harness.execution.llm_primitives._client_chat", return_value=mock_response):
            mock_client.return_value = MagicMock(model="test-model")
            from research_harness.execution.llm_primitives import algorithm_candidate_generate
    ...
    def test_with_deep_read_notes(self, db, conn):
        ...
        with patch("research_harness.execution.llm_primitives._get_client") as mock_client, \
            ...
            result = algorithm_candidate_generate(
                db=db, topic_id=1,
                brief={"problem_definition": "Test", "method_slots": []},
                deep_read_notes=[{"paper_id": 1, "title": "Paper 1", "summary": "Key finding"}],
            )
    ...
    def test_malformed_candidates_filtered(self, db, conn):
        ...
        mock_response = json.dumps({
            "candidates": [
                "not_a_dict",
                {"name": "ValidAlg", "architecture_description": "OK",
                 "components": [{"name": "c1", "role": "r", "provenance_tag": "novel"}],
                 "novelty_statement": "Novel", "feasibility_notes": "OK"},
            ],
...
class TestOriginalityBoundaryCheck:
    def test_novel_verdict(self, db, conn):
        _seed_topic(conn)
        _insert_paper(conn, 1, "Existing Method Paper", year=2024, compiled_summary="Uses DQN for bidding")
        conn.execute("INSERT INTO paper_topics (paper_id, topic_id, relevance) VALUES (1, 1, 'high')")
        conn.commit()
        ...
        mock_response = json.dumps({
            ...
            "near_matches": [
                {"paper_id": 1, "title": "Existing Method Paper",
                 "overlap_areas": ["Uses RL"], "differentiation": ["Novel attention decoder"]},
            ],
        ...
        with patch("research_harness.execution.llm_primitives._get_client") as mock_client, \
             patch("research_harness.execution.llm_primitives._client_chat", return_value=mock_response):
            mock_client.return_value = MagicMock(model="test-model")
            from research_harness.execution.llm_primitives import originality_boundary_check
            result = originality_boundary_check(
                db=db, topic_id=1,
                candidate={"name": "AdaptiveBidNet", "architecture_description": "Novel attention-based..."},
            )
    ...
    def test_too_similar_verdict(self, db, conn):
        ...
        mock_response = json.dumps({
            ...
            "near_matches": [
                {"paper_id": 5, "title": "Very Similar Paper",
        ...
        with patch("research_harness.execution.llm_primitives._get_client") as mock_client, \
            ...
            result = originality_boundary_check(
                db=db, topic_id=1,
                candidate={"name": "CopyAlg", "architecture_description": "Same as paper 5"},
            )
    ...
    def test_incremental_verdict(self, db, conn):
        ...
        with patch("research_harness.execution.llm_primitives._get_client") as mock_client, \
            ...
            result = originality_boundary_check(
                db=db, topic_id=1,
                candidate={"name": "IncrementalAlg", "architecture_description": "Minor extension"},
            )
...
class TestDesignPrompts:
    ...
    def test_candidate_generate_prompt_includes_theory_constraint(self):
        from research_harness.execution.prompts import algorithm_candidate_generate_prompt
    ...
    def test_prompts_handle_empty_optional_args(self):
        ...
        for fn in [design_brief_expand_prompt, design_gap_probe_prompt,
                    algorithm_candidate_generate_prompt, originality_boundary_check_prompt,
                    algorithm_design_refine_prompt]:
            system, user = fn("test")
...
class TestAlgorithmDesignRefine:
    def test_basic_refine(self, db, conn):
        ...
        mock_response = json.dumps({
            ...
            "components": [
                {"name": "state_encoder", "role": "encode", "provenance_tag": "borrowed",
                 "source_paper_id": 1, "details": "Transformer from paper 1"},
                {"name": "action_decoder", "role": "decode", "provenance_tag": "novel",
                 "source_paper_id": None, "details": "Novel attention decoder"},
            ],
            "novelty_statement": "Unlike prior work, our method uses attention-based allocation",
            "experiment_hooks": ["Compare vs PPO baseline", "Ablate attention module"],
            "provenance_summary": [
                {"component": "state_encoder", "origin": "borrowed", "source": "Paper 1"},
                {"component": "action_decoder", "origin": "novel", "source": "This work"},
            ],
        })
        ...
        with patch("research_harness.execution.llm_primitives._get_client") as mock_client, \
             patch("research_harness.execution.llm_primitives._client_chat", return_value=mock_response):
            mock_client.return_value = MagicMock(model="test-model")
            from research_harness.execution.llm_primitives import algorithm_design_refine
            result = algorithm_design_refine(
                db=db, topic_id=1,
                candidate={"name": "AdaptiveBidNet", "architecture_description": "A dual-stream..."},
                originality_result={"novelty_verdict": "novel", "novelty_score": 0.8},
            )

        assert result.proposal_title == "AdaptiveBidNet: Attention-Based Budget Allocation"
    ...
    def test_refine_malformed_components_filtered(self, db, conn):
        ...
        with patch("research_harness.execution.llm_primitives._get_client") as mock_client, \
            ...
            from research_harness.execution.llm_primitives import algorithm_design_refine
            result = algorithm_design_refine(
                db=db, topic_id=1, candidate={"name": "Test"},
            )
...
def _make_brief_response():
    return json.dumps({
        ...
        "method_slots": [{"name": "encoder", "role": "encode", "candidates": ["T"], "status": "open"}],
        "blocking_questions": [],
    })
...
def _make_gap_response(has_critical=False):
    gaps = []
    if has_critical:
        gaps.append({"slot": "encoder", "gap_type": "technique_unknown", "severity": "critical",
...
def _make_originality_response(verdict="novel", score=0.8):
    return json.dumps({
        ...
        "novelty_score": score,
        "recommended_modifications": [] if verdict == "novel" else ["Add unique component"],
    })
...
def _make_refine_response():
    return json.dumps({
        ...
        "provenance_summary": [{"component": "c1", "origin": "novel", "source": "This work"}],
    })
...
class TestAlgorithmDesignLoop:
    def test_converges_in_one_round(self, db, conn):
        ...
        with patch("research_harness.execution.llm_primitives._get_client") as mock_client, \
             patch("research_harness.execution.llm_primitives._client_chat", side_effect=mock_chat):
            mock_client.return_value = MagicMock(model="test-model")
            from research_harness.execution.llm_primitives import algorithm_design_loop
            result = algorithm_design_loop(
                db=db, topic_id=1, project_id=1, direction="Test direction",
            )
    ...
    def test_max_rounds_hit(self, db, conn):
        ...
        with patch("research_harness.execution.llm_primitives._get_client") as mock_client, \
            ...
            from research_harness.execution.llm_primitives import algorithm_design_loop
            result = algorithm_design_loop(
                db=db, topic_id=1, project_id=1, direction="Test",
                max_rounds=2,
            )
    ...
    def test_no_candidates_early_stop(self, db, conn):
        ...
        responses = [
            _make_brief_response(),
            _make_gap_response(has_critical=False),
            json.dumps({"candidates": [], "method_inventory_used": 0}),  # empty candidates
        ]
        call_idx = {"i": 0}
...
class TestDesignHarnessActions:
    ...
    def test_design_actions_chain_correctly(self):
        ...
        assert "algorithm_candidate_generate" in STATIC_NEXT_ACTIONS["design_gap_probe"][0]
        assert "originality_boundary_check" in STATIC_NEXT_ACTIONS["algorithm_candidate_generate"][0]
        assert "algorithm_design_refine" in STATIC_NEXT_ACTIONS["originality_boundary_check"][0]
...
class TestDesignIntegration:
    ...
    def test_all_design_primitives_in_registry(self):
        ...
        design_primitives = [
            "design_brief_expand", "design_gap_probe",

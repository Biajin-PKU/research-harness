...
class TestReadingPrioritize:
    def test_ranks_by_composite_score(self, db, conn):
        ...
        _insert_paper(conn, 1, "Old High-Cited Paper", year=2018, citation_count=500, status="annotated")
        # Paper 2: low citations, recent
        _insert_paper(conn, 2, "New Low-Cited Paper", year=2025, citation_count=5, status="meta_only")
        # Paper 3: medium everything
        _insert_paper(conn, 3, "Medium Paper", year=2022, citation_count=50, status="annotated")
        ...
        conn.execute("INSERT INTO paper_topics (paper_id, topic_id, relevance) VALUES (3, 1, 'medium')")
        conn.commit()
    ...
    def test_limit(self, db, conn):
        ...
        for i in range(1, 6):
            ...
            conn.execute(f"INSERT INTO paper_topics (paper_id, topic_id, relevance) VALUES ({i}, 1, 'medium')")
        conn.commit()
    ...
    def test_custom_weights(self, db, conn):
        ...
        conn.execute("INSERT INTO paper_topics (paper_id, topic_id, relevance) VALUES (1, 1, 'medium')")
        conn.execute("INSERT INTO paper_topics (paper_id, topic_id, relevance) VALUES (2, 1, 'medium')")
        conn.commit()

        from research_harness.primitives.analysis_impls import reading_prioritize

        # Weight heavily toward recency
        result = reading_prioritize(db=db, topic_id=1, weights={"gap": 0, "citation": 0, "recency": 1})
        assert result.ranked[0].paper_id == 2  # newer paper wins

        # Weight heavily toward citations
        result = reading_prioritize(db=db, topic_id=1, weights={"gap": 0, "citation": 1, "recency": 0})
...
class TestExperimentDesignChecklist:
    def test_returns_checklist(self, db, conn):
        ...
        from research_harness.primitives.analysis_impls import experiment_design_checklist
    ...
    def test_fills_notes_from_compiled_summaries(self, db, conn):
        ...
        has_method_notes = any("DQN" in i.notes or "PPO" in i.notes for i in baseline_items)
...
class TestMethodTaxonomy:
    def test_builds_taxonomy(self, db, conn):
        conn.execute("INSERT INTO topics (id, name) VALUES (1, 'test')")
        compiled = json.dumps({
            "overview": "RL for bidding", "methods": ["DQN", "PPO"],
            "claims": [], "limitations": [], "metrics": [], "relations": [],
        })
        ...
        mock_response = json.dumps({"nodes": [
            {"name": "Reinforcement Learning", "parent": None, "description": "RL category",
             "aliases": ["RL"], "paper_ids": [1]},
            {"name": "DQN", "parent": "Reinforcement Learning", "description": "Deep Q-Network",
             "aliases": ["Deep Q-Network"], "paper_ids": [1]},
        ]})
        ...
        conn2 = db.connect()
        nodes = conn2.execute("SELECT * FROM taxonomy_nodes WHERE topic_id = 1").fetchall()
        conn2.close()
        assert len(nodes) == 2
...
class TestEvidenceMatrix:
    def test_normalizes_claims(self, db, conn):
        ...
        compiled = json.dumps({
            ...
            "methods": ["DQN", "PPO"],
            "claims": [{"claim": "DQN outperforms PPO on CTR", "evidence": "Table 3", "strength": "strong"}],
            "limitations": [],
            "metrics": [{"dataset": "AdsData", "metric": "CTR", "value": "0.85", "baseline": "0.80"}],
            "relations": [],
        })
        ...
        mock_response = json.dumps({"normalized_claims": [
            {"paper_id": 1, "claim_text": "DQN outperforms PPO on CTR",
             "method": "DQN", "dataset": "AdsData", "metric": "CTR",
             "task": "ad bidding", "value": "0.85", "direction": "higher_better",
             "confidence": 0.8},
        ]})
        ...
        # Check DB persistence
        conn2 = db.connect()
        rows = conn2.execute("SELECT * FROM normalized_claims WHERE topic_id = 1").fetchall()
        conn2.close()
        assert len(rows) == 1
...
class TestContradictionDetect:
    def test_detects_contradictions(self, db, conn):
        ...
        mock_response = json.dumps({"contradictions": [
            {"claim_a_id": 1, "claim_b_id": 2, "same_task": True, "same_dataset": True,
             "same_metric": True, "confidence": 0.9,
        ...
        # Check DB persistence
        conn2 = db.connect()
        rows = conn2.execute("SELECT * FROM contradictions WHERE topic_id = 1").fetchall()
        conn2.close()
        assert len(rows) == 1
...
class TestMigration022:
    def test_tables_exist(self, db, conn):
        for table in ("taxonomy_nodes", "taxonomy_assignments", "normalized_claims", "contradictions"):
...
class TestSpecRegistration:
    def test_all_phase2_specs_registered(self):
        ...
        for name in ("reading_prioritize", "method_taxonomy", "experiment_design_checklist",
                      "evidence_matrix", "contradiction_detect"):

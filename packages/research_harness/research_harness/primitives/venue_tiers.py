...
_CCF_SCORES = {CCFTier.A_STAR: 100, CCFTier.A: 90, CCFTier.B: 70, CCFTier.C: 40, CCFTier.NONE: 0}
_CAS_SCORES = {CASQuartile.Q1: 85, CASQuartile.Q2: 65, CASQuartile.Q3: 40, CASQuartile.Q4: 20, CASQuartile.NONE: 0}
...
class VenueTier:
    ...
    @staticmethod
    def create(ccf: CCFTier = CCFTier.NONE, cas: CASQuartile = CASQuartile.NONE) -> VenueTier:
...
def _t(ccf: str = "", cas: str = "") -> VenueTier:
    ...
    c = {"A*": CCFTier.A_STAR, "A": CCFTier.A, "B": CCFTier.B, "C": CCFTier.C}.get(ccf, CCFTier.NONE)
    q = {"Q1": CASQuartile.Q1, "Q2": CASQuartile.Q2, "Q3": CASQuartile.Q3, "Q4": CASQuartile.Q4}.get(cas, CASQuartile.NONE)
...
_VENUE_ENTRIES: list[tuple[list[str], VenueTier]] = [
    ...
    (["neurips", "nips", "advances in neural information processing systems"], _t("A*")),
    ...
    (["cvpr", "ieee/cvf conference on computer vision and pattern recognition",
      "ieee conference on computer vision and pattern recognition"], _t("A")),
    (["iccv", "ieee/cvf international conference on computer vision",
      "international conference on computer vision"], _t("A")),
    # NLP
    (["acl", "annual meeting of the association for computational linguistics",
      "association for computational linguistics"], _t("A")),
    # Data
    (["sigmod", "acm sigmod", "international conference on management of data"], _t("A")),
    (["vldb", "pvldb", "proceedings of the vldb endowment",
      "very large data bases"], _t("A")),
    (["icde", "ieee international conference on data engineering"], _t("A")),
    (["sigir", "acm sigir", "international acm sigir conference on research and development in information retrieval"], _t("A")),
    (["kdd", "acm sigkdd", "knowledge discovery and data mining"], _t("A")),
    # Web
    (["www", "the web conference", "world wide web conference",
      "international world wide web conference"], _t("A")),
    # Networks
    (["sigcomm", "acm sigcomm"], _t("A")),
    (["mobicom", "acm mobicom", "international conference on mobile computing and networking"], _t("A")),
    (["infocom", "ieee infocom"], _t("A")),
    # Security
    (["ccs", "acm ccs", "acm conference on computer and communications security"], _t("A")),
    ...
    # Systems
    (["osdi", "usenix symposium on operating systems design and implementation"], _t("A")),
    ...
    (["hpca", "ieee international symposium on high-performance computer architecture"], _t("A")),
    # PL
    (["pldi", "acm sigplan conference on programming language design and implementation"], _t("A")),
    ...
    (["fse", "acm sigsoft symposium on the foundation of software engineering",
      "foundations of software engineering"], _t("A")),
    (["ase", "ieee/acm international conference on automated software engineering"], _t("A")),
    # HCI
    (["chi", "acm chi", "acm conference on human factors in computing systems"], _t("A")),
    (["ubicomp", "acm international joint conference on pervasive and ubiquitous computing"], _t("A")),
    # Graphics
    ...
    (["wacv", "ieee/cvf winter conference on applications of computer vision"], _t("B")),
    (["bmvc", "british machine vision conference"], _t("B")),
    # NLP
    (["emnlp", "conference on empirical methods in natural language processing"], _t("B")),
    (["naacl", "north american chapter of the association for computational linguistics"], _t("B")),
    (["coling", "international conference on computational linguistics"], _t("B")),
    (["eacl", "european chapter of the association for computational linguistics"], _t("B")),
    # Data/IR
    (["cikm", "acm international conference on information and knowledge management"], _t("B")),
    ...
    # AI/ML
    (["aamas", "international conference on autonomous agents and multiagent systems"], _t("B")),
    (["uai", "conference on uncertainty in artificial intelligence"], _t("B")),
    (["aistats", "international conference on artificial intelligence and statistics"], _t("B")),
    (["colt", "conference on learning theory"], _t("B")),
    (["ecai", "european conference on artificial intelligence"], _t("B")),
    (["pakdd", "pacific-asia conference on knowledge discovery and data mining"], _t("B")),
    # Systems
    (["asplos", "international conference on architectural support for programming languages and operating systems"], _t("B")),
    (["eurosys", "european conference on computer systems"], _t("B")),
    (["middleware", "acm/ifip international middleware conference"], _t("B")),
    (["sc", "supercomputing", "international conference for high performance computing"], _t("B")),
    (["ppopp", "acm sigplan symposium on principles and practice of parallel programming"], _t("B")),
    # Networks
    (["imc", "acm internet measurement conference"], _t("B")),
    (["conext", "acm international conference on emerging networking experiments and technologies"], _t("B")),
    # Security
    (["raid", "international symposium on research in attacks, intrusions and defenses"], _t("B")),
    ...
    (["saner", "ieee international conference on software analysis, evolution and reengineering"], _t("B")),
    (["icsme", "ieee international conference on software maintenance and evolution"], _t("B")),
    (["msr", "mining software repositories"], _t("B")),
    # Robotics
    (["icra", "ieee international conference on robotics and automation"], _t("B")),
    (["iros", "ieee/rsj international conference on intelligent robots and systems"], _t("B")),
    (["corl", "conference on robot learning"], _t("B")),
    # Graphics
    (["eurographics", "eg", "annual conference of the european association for computer graphics"], _t("B")),
    # HCI
    (["uist", "acm symposium on user interface software and technology"], _t("B")),
    (["cscw", "acm conference on computer-supported cooperative work and social computing"], _t("B")),
    # Multimedia
    ...
    (["pricai", "pacific rim international conference on artificial intelligence"], _t("C")),
    (["iconip", "international conference on neural information processing"], _t("C")),
    (["ksem", "international conference on knowledge science, engineering and management"], _t("C")),

    # === Journals — CCF + CAS combined ===
    # CCF-A Journals
    (["tpami", "ieee transactions on pattern analysis and machine intelligence"], _t("A", "Q1")),
    ...
    (["tosem", "acm transactions on software engineering and methodology"], _t("A", "Q1")),
    (["tochi", "acm transactions on computer-human interaction"], _t("A", "Q1")),
    (["tog", "acm transactions on graphics"], _t("A", "Q1")),
    (["tifs", "ieee transactions on information forensics and security"], _t("A", "Q1")),
    (["tdsc", "ieee transactions on dependable and secure computing"], _t("A", "Q1")),
    (["tc", "ieee transactions on computers"], _t("A", "Q2")),
    (["tcad", "ieee transactions on computer-aided design of integrated circuits and systems"], _t("A", "Q2")),
    ...
    (["ipm", "information processing and management", "information processing & management"], _t("B", "Q1")),
    ...
    (["tnnls", "ieee transactions on neural networks and learning systems"], _t("B", "Q1")),
    ...
    (["taslp", "ieee/acm transactions on audio, speech and language processing",
      "ieee/acm transactions on audio speech and language processing"], _t("B", "Q1")),
    (["tacl", "transactions of the association for computational linguistics"], _t("B", "Q1")),
    (["cl", "computational linguistics"], _t("B", "Q1")),
    (["tvcg", "ieee transactions on visualization and computer graphics"], _t("B", "Q1")),
    (["jair", "journal of artificial intelligence research"], _t("B", "Q2")),
    (["ml", "machine learning"], _t("B", "Q2")),
    (["dke", "data & knowledge engineering", "data and knowledge engineering"], _t("B", "Q3")),
